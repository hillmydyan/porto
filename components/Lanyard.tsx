...
export default function Lanyard({
  ...
  return (
    <div className="w-full h-full relative z-[60] pointer-events-none">
      <Canvas
        camera={{ position: isMobile ? [0, 0, 19] : position, fov }}
...
function Band({ maxSpeed = 50, minSpeed = 0, isMobile = false }: BandProps) {
  ...
  // Memoize resolution Vector2 so MeshLineMaterial uniform does not re-compile every frame
  const resVector = useMemo(() => new THREE.Vector2(size.width, size.height), [size.width, size.height]);
  ...
  useFrame((state, delta) => {
    ...
    if (fixed.current && j1.current && j2.current && j3.current) {
      // Direct physics translation sync for instant 60fps responsiveness
